...
cam_mtx = np.load(camera_data_path + 'cam_mtx.npy')
dist = np.load(camera_data_path + 'dist.npy')
newcam_mtx = np.load(camera_data_path + 'newcam_mtx.npy')
roi = np.load(camera_data_path + 'roi.npy')
...
worldPoints = np.array(
    [[X_center, Y_center, Z_center], [5.5, 3.9, 46.8], [14.2, 3.9, 47.0],
     [22.8, 3.9, 47.4], [5.5, 10.6, 44.2], [14.2, 10.6, 43.8],
     [22.8, 10.6, 44.8], [5.5, 17.3, 43], [14.2, 17.3, 42.5],
     [22.8, 17.3, 44.4]],
    dtype=np.float32)
...
imagePoints = np.array(
    [[cx, cy], [502, 185], [700, 197], [894, 208], [491, 331], [695, 342],
     [896, 353], [478, 487], [691, 497], [900, 508]],
    dtype=np.float32)
...
if writeValues:
    np.save(camera_data_path + 'inverse_newcam_mtx.npy', inverse_newcam_mtx)
...
print(rvec1)
if writeValues:
        np.save(camera_data_path + 'rvec1.npy', rvec1)

print("pnp tvec1 - Translation")
print(tvec1)
if writeValues:
        np.save(camera_data_path + 'tvec1.npy', tvec1)

print("R - rodrigues vecs")
R_mtx, jac = cv2.Rodrigues(rvec1)
print(R_mtx)
if writeValues:
        np.save(camera_data_path + 'R_mtx.npy', R_mtx)

print("R|t - Extrinsic Matrix")
Rt = np.column_stack((R_mtx, tvec1))
print(Rt)
if writeValues:
        np.save(camera_data_path + 'Rt.npy', Rt)

print("newCamMtx*R|t - Projection Matrix")
P_mtx = newcam_mtx.dot(Rt)
print(P_mtx)
if writeValues:
        np.save(camera_data_path + 'P_mtx.npy', P_mtx)
...
for i in range(0, total_points_used):
    ...
    s_describe[i] = s
    if writeValues:
            np.save(camera_data_path + 's_arr.npy', s_arr)

    print("Solve: From Image Pixels, find World Points")

    uv_1 = np.array([[imagePoints[i, 0], imagePoints[i, 1], 1]],
                    dtype=np.float32)
    uv_1 = uv_1.T
    print(">==> uv1")
    print(uv_1)
...
for i in range(0, total_points_used):
    print("Point " + str(i))
    print("S: " + str(s_describe[i]) + " Mean: " + str(s_mean) + " Error: " +
          str(s_describe[i] - s_mean))